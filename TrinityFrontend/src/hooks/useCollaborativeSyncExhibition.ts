...
const DEFAULT_OPTIONS: Required<CollaborativeSyncOptions> = {
  ...
  onError: (error) => console.error('[ExhibitionCollaborativeSync]', error),
  onConnected: () => console.log('[ExhibitionCollaborativeSync] Connected'),
  onDisconnected: () => console.log('[ExhibitionCollaborativeSync] Disconnected'),
  onUsersChanged: () => {},
};

/**
 * Hook for real-time collaborative synchronization using WebSocket for Exhibition Mode
 * 
 * Features:
 ...
 */
export function useCollaborativeSyncExhibition(options: CollaborativeSyncOptions = {}) {
  ...
  const lastSlideObjectsRef = useRef<Record<string, any[]>>({});  // Track previous slide objects for diff
  ...
  const cards = useExhibitionStore(state => state.cards);
  ...
  const updateCard = useExhibitionStore(state => state.updateCard);
  const { user } = useAuth();
  ...
  const getWebSocketUrl = useCallback(() => {
    ...
    const wsUrl = `${baseUrl}/api/exhibition/sync/${projectContext.client_name}/${projectContext.app_name}/${projectContext.project_name}`;
    return wsUrl;
  }, []);
  ...
  const serializeState = useCallback(() => {
    try {
      ...
      const exhibitionConfig = {
        cards: cards || [],
        slide_objects: slideObjectsByCardId || {},
        timestamp: new Date().toISOString(),
      };
      
      return {
        config: exhibitionConfig,
        serialized: safeStringify(exhibitionConfig),
        projectContext,
      };
    } catch (error) {
  ...
  const sendStateUpdate = useCallback(() => {
    ...
    // Also check if slide_objects changed
    const slideObjectsChanged = safeStringify(slideObjectsByCardId) !== safeStringify(lastSlideObjectsRef.current);
    
    if (changedCards.length === 0 && !countChanged && !slideObjectsChanged) {
      return;
    }
    ...
    lastSlideObjectsRef.current = slideObjectsByCardId || {};
    
    // If cards were added/deleted or slide_objects changed, send full sync to avoid conflicts
    if (countChanged || slideObjectsChanged) {
  ...
  }, [cards, slideObjectsByCardId, getChangedCards, sendMessage, serializeState]);
      ...
      switch (message.type) {
        case 'card_update':
          ...
          if (message.card_id && message.payload) {
            ...
            if (lastLocalChangeTimestampRef.current > 0 && 
              ...
              console.log('[ExhibitionCollaborativeSync] Ignoring stale card_update', {
        ...
        case 'full_sync':
          if (message.payload) {
            ...
            if (lastLocalChangeTimestampRef.current > 0 && 
                messageTimestamp < lastLocalChangeTimestampRef.current - 500) {
              console.log('[ExhibitionCollaborativeSync] Ignoring stale full_sync', {
                messageTime: messageTimestamp,
                lastLocalChange: lastLocalChangeTimestampRef.current,
              });
              break;
            ...
            isApplyingRemoteUpdateRef.current = true;
            if (message.payload.cards && message.payload.slide_objects) {
              setCards(message.payload.cards, message.payload.slide_objects);
              lastCardsRef.current = message.payload.cards;
              lastSlideObjectsRef.current = message.payload.slide_objects;
            }
        ...
        default:
          console.warn('[ExhibitionCollaborativeSync] Unknown message type:', message.type);
      }
    } catch (error) {
      onErrorRef.current(error as Error);
    }
  }, [setCards, serializeState, sendMessage, updateCard, cards, getUserColor]);
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      console.log('[ExhibitionCollaborativeSync] Connecting to:', wsUrl);
      ...
      ws.onopen = () => {
        console.log('[ExhibitionCollaborativeSync] Connected');
        setIsConnected(true);
        ...
        setTimeout(() => {
          const latestCards = useExhibitionStore.getState().cards || [];
          const latestSlideObjects = useExhibitionStore.getState().slideObjectsByCardId || {};
          if (latestCards.length > 0 && !hasInitialFullSyncRef.current) {
            hasInitialFullSyncRef.current = true;
            sendFullSyncRef.current();
            console.log('[ExhibitionCollaborativeSync] Initial full sync sent with', latestCards.length, 'cards');
          }
        }, 100);
      ...
      ws.onerror = (error) => {
        console.error('[ExhibitionCollaborativeSync] WebSocket error:', error);
        onErrorRef.current(new Error('WebSocket connection error'));
      };

      ws.onclose = () => {
        console.log('[ExhibitionCollaborativeSync] Disconnected');
        ...
        if (!manualCloseRef.current && enabled) {
          reconnectTimeoutRef.current = setTimeout(() => {
            console.log('[ExhibitionCollaborativeSync] Attempting reconnection...');
            connect();
          }, 3000);
        }
      };
  ...
  }, [cards, slideObjectsByCardId, enabled, debounceMs, sendStateUpdate]);
