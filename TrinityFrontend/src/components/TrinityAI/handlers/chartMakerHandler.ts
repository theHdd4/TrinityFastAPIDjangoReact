...
export const chartMakerHandler: AtomHandler = {
  handleSuccess: async (data: any, context: AtomHandlerContext): Promise<AtomHandlerResponse> => {
    ...
    if (!chartJson) {
      ...
      // Even for file listing, call insights with the 3 keys
      console.log('🔍 No chart config - Still calling insights with 3 keys...');
    ...
    try {
      ...
      if (loadResponse.ok) {
        ...
        // 🔍 IMMEDIATELY call insights after chart is rendered and 3 keys are shown
        // Wait a moment to ensure previous connection is closed, then open new connection to LLM
        await new Promise(resolve => setTimeout(resolve, 500)); // Small delay to ensure connection is closed
        
        // Open new connection to LLM for insight generation
        console.log('\n' + '='.repeat(80));
        console.log('🔍 STEP: Chart rendered - Opening NEW connection to LLM for insights');
        console.log('='.repeat(80));
        console.log('📤 Sending insight request with:');
        console.log('  - Smart Response:', data.smart_response?.substring(0, 100) || 'N/A');
        console.log('  - Response:', data.response?.substring(0, 100) || 'N/A');
        console.log('  - Reasoning:', data.reasoning?.substring(0, 100) || 'N/A');
        console.log('  - Atom Type: chart-maker');
        console.log('  - Session ID:', sessionId);
        console.log('='.repeat(80) + '\n');
        
        // Call insights immediately - this opens a new HTTP connection to the insight API
        try {
          const result = await generateAndFormatInsight({
            data,
            atomType: 'chart-maker',
            sessionId,
          });
          console.log('\n' + '='.repeat(80));
          console.log('✅ STEP: Insight received from LLM');
          console.log('='.repeat(80));
          console.log('📥 Insight content:', result.insight);
          console.log('='.repeat(80) + '\n');
          
          // Update card's text box with formatted insight content (below the 3 keys)
          try {
            await updateCardTextBox(atomId, result.formattedContent);
            console.log('✅ Card text box updated with insight (below 3 keys in text box)');
          ...
          console.error('\n' + '='.repeat(80));
          console.error('❌ ERROR: Insight generation failed');
        ...
        // Even if chart loading fails, still call insights with the 3 keys
        console.log('🔍 Chart loading failed, but still calling insights with 3 keys...');
        generateAndFormatInsight({
          data,
          atomType: 'chart-maker',
          sessionId,
