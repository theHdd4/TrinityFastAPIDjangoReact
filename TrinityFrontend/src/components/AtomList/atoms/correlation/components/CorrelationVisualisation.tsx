...
import { X, AlertCircle, Loader2, Calendar, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { format, parse, isValid, getDaysInMonth, startOfMonth, addMonths } from 'date-fns';
...
const CorrelationVisualisation: React.FC<CorrelationVisualisationProps> = ({ data, onDataChange }) => {
  ...
  const runCorrelationAnalysis = async (filePath: string) => {
    try {
      ...
      const matrix = resultVariables.map((rowVar: string, i: number) => {
        return resultVariables.map((colVar: string, j: number) => {
          if (i === j) return 1.0;
          const value = correlationDict[rowVar]?.[colVar];
          return typeof value === 'number' && !isNaN(value) && isFinite(value) ? value : 0.0;
        });
      });
      ...
      onDataChange({
        ...
        fileData: {
          ...
          dateColumns:
            result.date_analysis?.date_columns.map((col: any) => col.column_name) ||
            data.fileData?.dateColumns || [],
          categoricalColumns:
            data.fileData?.categoricalColumns ||
            (result.columns_used || []).filter(
              (col: string) => !resultVariables.includes(col),
            ),
      ...
      setProcessingError('Failed to apply correlation analysis');
    }
  };
