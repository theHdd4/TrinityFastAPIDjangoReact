...
const MetricsColOps = forwardRef<MetricsColOpsRef, MetricsColOpsProps>(({ dataSource, featureOverviewApi, onColumnCreated, onTableCreated, columnOperationsState, onColumnOperationsStateChange, onPreviewReady }, ref) => {
  ...
  // Get laboratory store for table atom auto-display
  ...
  const handleTableAtomAutoDisplay = async (
    ...
    try {
      ...
      console.error('[MetricsColOps GuidedFlow] Table auto-display error:', tableError);
      ...
      try {
        ...
        console.error('[MetricsColOps GuidedFlow] Table auto-display fallback error:', fallbackError);
      }
    }
  };
  ...
  const getOutputColName = (op: typeof selectedOperations[0]) => {
    ...
    switch (op.type) {
      ...
      case 'multiply': result = columns.join('_x_'); break;
      // Backend: "_div_".join(columns)
      case 'divide': result = columns.join('_div_'); break;
      ...
      // Backend: f"res_{y_var}" - use lowercase
