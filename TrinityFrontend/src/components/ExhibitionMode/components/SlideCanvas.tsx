...
interface SlideCanvasProps {
  ...
  totalSlides: number;
  onDrop: () => void;
  renderAtomContent?: (atom: DroppedAtom) => React.ReactNode;
}
...
export const SlideCanvas: React.FC<SlideCanvasProps> = ({
  ...
  const getSlideDescription = () => {
    if (card.atoms.length > 0) {
      return `Explore ${card.atoms.length} ${card.atoms.length === 1 ? 'component' : 'components'} with our comprehensive analysis and insights. Stay organized and focused on key findings and activities.`;
    }
      ...
      >
        <div
          className={cn(
            ...
            isDragOver && 'scale-[0.98] ring-4 ring-primary/20'
          )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <div className="flex flex-col gap-0.5 w-6 h-6">
                        <div className="h-2 border-2 border-current rounded-sm"></div>
                        <div className="h-3 border-2 border-current rounded-sm bg-current/20"></div>
                      </div>
                    </Button>
                    <Button
                      size="icon"
                    ...
                    >
                      <div className="flex gap-0.5 w-6 h-6">
                        <div className="w-2 border-2 border-current rounded-sm"></div>
                        <div className="w-3 border-2 border-current rounded-sm bg-current/20"></div>
                      </div>
                    </Button>
                    <Button
                      size="icon"
                    ...
                    >
                      <div className="flex gap-0.5 w-6 h-6">
                        <div className="w-2 border-2 border-current rounded-sm"></div>
                        <div className="flex-1 border-2 border-current rounded-sm bg-current/20"></div>
                      </div>
                    </Button>
                    <Button
                      size="icon"
                    ...
                    >
                      <div className="w-6 h-6 border-2 border-current rounded bg-current/20"></div>
                    </Button>
                  </div>
                </div>
                  ...
                  {card.atoms.map((atom) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        <div className={`w-3 h-3 ${atom.color} rounded-full flex-shrink-0`}></div>
                      ...
                      <div className="text-sm text-muted-foreground space-y-3">
                        {renderAtomContent ? (
                          <div className="rounded-lg border border-dashed border-border/60 p-3 bg-muted/40">
