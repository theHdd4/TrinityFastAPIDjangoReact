                ...
                <div className="ml-6 mt-2 space-y-1">
                  {card.atoms.map((atom) => (
                    <div
                      key={atom.id}
                      draggable={!disabled}
                      onDragStart={() => !disabled && onDragStart(atom, card.id)}
                      className={cn(
                        'flex items-center gap-2 px-2 py-1.5 rounded bg-muted/50 transition-colors',
                        disabled ? 'cursor-not-allowed opacity-60' : 'hover:bg-muted cursor-move group'
                      )}
                    >
