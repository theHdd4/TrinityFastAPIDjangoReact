...
class StreamSequenceGenerator:
    ...
    async def _call_llm(self, prompt: str, temperature: float = 0.3) -> str:
        ...
        print("\n" + "="*80)
        print("🚀 STREAMAI SEQUENCE GENERATOR LLM CALL - FULL PROMPT")
        print("="*80)
        print(f"API URL: {self.api_url}")
        print(f"Model: {self.model_name}")
        print(f"Temperature: {temperature}, Num Predict: 2000")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    print("\n" + "="*80)
                    print("📥 STREAMAI SEQUENCE GENERATOR LLM - RAW RESPONSE")
                    print("="*80)
                    ...
                    print("\n" + "="*80)
                    print("✨ STREAMAI SEQUENCE GENERATOR LLM - PROCESSED CONTENT")
                    print("="*80)
    ...
    def _format_basic_file_details(self, file_details_dict: Dict[str, Dict[str, Any]]) -> str:
        """
        Format basic file details (from FileReader) for LLM prompt.
        
        Args:
            file_details_dict: Dictionary mapping filenames to basic metadata (columns only)
        
        Returns:
            Formatted string for LLM prompt
        """
        lines = []
        lines.append("**📊 FILE METADATA (Basic - Columns Only):**")
        lines.append("")
        
        for filename, metadata in file_details_dict.items():
            columns = metadata.get("columns", [])
            column_count = metadata.get("column_count", 0)
            
            lines.append(f"**File: {filename}**")
            lines.append(f"- **Columns ({column_count}):** {', '.join(columns) if columns else 'No columns available'}")
            lines.append("")
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error("❌ BaseAgent.FileReader not available - cannot retrieve file details")
                        FileReader = None
                ...
                # Get file details using BaseAgent.FileReader
                if file_names and FileReader is not None:
                    file_details_dict = {}
                    try:
                        logger.info(f"📥 Loading file details for {len(file_names)} file(s) using BaseAgent.FileReader")
                        ...
                        if file_details_dict:
                            # Format file details for prompt
                            comprehensive_file_details = self._format_basic_file_details(file_details_dict)
                            file_details_for_aliases = file_details_dict
                            file_details_loaded = file_details_dict
                            logger.info(f"✅ Loaded file details for {len(file_details_dict)} file(s)")
                        else:
                            logger.debug("⚠️ Could not load file details, using basic file list")
                            comprehensive_file_details = self._build_basic_file_list(files_list)
                    except Exception as e:
                        logger.debug(f"⚠️ Failed to get file details: {e}, using basic file list")
                        comprehensive_file_details = self._build_basic_file_list(files_list)
                else:
                    # No file names or FileReader not available
                    comprehensive_file_details = self._build_basic_file_list(files_list)
            except Exception as e:
                # Log as debug since this is non-critical (files can still be accessed via FileReader)
                logger.debug(f"⚠️ Could not load file details: {e} (non-critical - using basic file list)")
                comprehensive_file_details = self._build_basic_file_list(files_list)
        else:
            comprehensive_file_details = ""
