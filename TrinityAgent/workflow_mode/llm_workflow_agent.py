...
# Import centralized config from BaseAgent
try:
...
# Import FileReader from BaseAgent (standardized file handler)
try:
...
class WorkflowCompositionAgent:
    ...
    def __init__(self, api_url: str, model_name: str, bearer_token: str):
        ...
        # Initialize FileReader from BaseAgent (standardized file handler)
        self.file_reader = None
        if FILE_READER_AVAILABLE and FileReader is not None:
            try:
                # FileReader uses centralized config automatically
                self.file_reader = FileReader()
                logger.info("✅ FileReader initialized from BaseAgent for @filename mention support")
    ...
    def process_request(
        ...
        if file_context is None and self.file_reader:
            try:
                # Parse @filename mentions from user prompt
                import re
                filename_pattern = r'@(\w+\.\w+)'  # Match @filename.ext
                mentioned_files = re.findall(filename_pattern, user_prompt)
                
                if mentioned_files:
                    # Load files using FileReader
                    # Note: FileReader will use default prefix or context from settings
                    files_data = self.file_reader.load_files(
                        client_name="",
                        app_name="",
                        project_name=""
                    )
                    ...
                    for filename in mentioned_files:
                        # Find matching file in loaded files
                        for file_path, file_data in files_data.items():
                            if filename in file_path or filename in file_data.get("file_name", ""):
