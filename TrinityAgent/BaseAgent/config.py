"""
Centralized Configuration for Trinity AI Base Agent
Replaces scattered os.getenv calls with a single, type-safe Pydantic configuration model.
"""
...
class Settings(BaseSettings):
    ...
    POSTGRES_DB: str = Field(default="trinity_prod", description="PostgreSQL database name")
    POSTGRES_USER: str = Field(default="trinity_user", description="PostgreSQL user")
    POSTGRES_PASSWORD: str = Field(default="trinity_pass", description="PostgreSQL password")
    ...
    if PYDANTIC_V2:
        model_config = SettingsConfigDict(
            ...
            extra="ignore"  # Ignore extra environment variables (like Django settings)
        )
    else:
        class Config:
            env_file = ".env"
