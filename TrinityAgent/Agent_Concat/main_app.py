"""
Standardized Concat Agent using BaseAgent infrastructure
...
try:
    try:
        ...
        from .concat_prompt import ConcatPromptBuilder
...
# Only define ConcatAgent if BaseAgent was imported successfully
if BaseAgent is not None:
    class ConcatAgent(BaseAgent):
        """
        Standardized Concat Agent using BaseAgent infrastructure.
        Only implements concat-specific logic.
        """
        ...
        def description(self) -> str:
            return "Concatenates data files vertically (stacking rows) or horizontally (appending columns)"
        ...
        def _call_llm(self, prompt: str, temperature: float = 0.1, num_predict: int = 4000) -> str:
            ...
            logger.info("=" * 80)
            logger.info("📥 RECEIVED FROM CONCAT LLM:")
            logger.info("=" * 80)
        ...
        def _build_prompt(
            ...
            """
            Build concat-specific prompt using ConcatPromptBuilder.
            BaseAgent handles file loading and context building automatically.
            """
            # Use ConcatPromptBuilder to build the prompt
            # BaseAgent's execute() method provides available_files and context
            prompt = ConcatPromptBuilder.build_concat_prompt(
            ...
            logger.info("=" * 80)
            logger.info("📤 SENDING TO CONCAT LLM:")
            logger.info("=" * 80)
        ...
        def _validate_json(self, result: Dict[str, Any]) -> bool:
            """
            Validate concat-specific JSON structure.
            BaseAgent handles general validation, this adds concat-specific checks.
            """
            if not isinstance(result, dict):
                return False
            
            # If success is True, must have concat_json
            if result.get("success") is True:
                if "concat_json" not in result:
                    return False
                ...
                if "file1" not in concat_json or "file2" not in concat_json:
                    return False
                
                # Must have concat_direction
                if "concat_direction" not in concat_json:
                    return False
                
                direction = concat_json.get("concat_direction")
        ...
        def _normalize_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
            """
            Normalize concat result to ensure consistent format.
            ...
            # Add concat_json if present
            if "concat_json" in result:
                concat_json = result["concat_json"]
                normalized["concat_json"] = {
        ...
        def _create_fallback_response(self, session_id: str) -> Dict[str, Any]:
            ...
            return {
                ...
                "suggestions": [
                    ...
                    "Example: 'concatenate file1.csv with file2.csv vertically'"
                ],
    ...
    # Define ConcatAgent as None if BaseAgent is not available
    ConcatAgent = None
...
# ============================================================================
logger.info("=" * 80)
logger.info("CONCAT AGENT MODULE LOADING")
logger.info("=" * 80)
...
if BaseAgent is not None and settings is not None:
    logger.info("=" * 80)
    logger.info("INITIALIZING CONCAT AGENT")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info("Creating ConcatAgent instance...")
        agent = ConcatAgent(
        ...
        logger.info("✅ ConcatAgent initialized successfully")
        ...
        try:
            ...
            logger.info(f"✅ Registered ConcatAgent in BaseAgent registry")
        except ImportError:
            try:
                from TrinityAgent.BaseAgent.registry import registry
                registry.register(agent)
                logger.info(f"✅ Registered ConcatAgent in BaseAgent registry (absolute import)")
        ...
        logger.error("=" * 80)
        logger.error(f"❌ Failed to initialize ConcatAgent: {e}", exc_info=True)
        logger.error("=" * 80)
        agent = None
        agent_initialized = False
else:
...
def _retry_baseagent_import():
    ...
    global BaseAgent, AgentContext, AgentResult, settings, ConcatPromptBuilder
    ...
    try:
        try:
            ...
            from BaseAgent import BaseAgent, AgentContext, AgentResult, settings, ConcatPromptBuilder
...
class ConcatRequest(BaseModel):
    """Request model for concat endpoint."""
    prompt: str
...
@router.post("/concat")
def concatenate_files(request: ConcatRequest) -> Dict[str, Any]:
    """
    ...
    global agent, agent_initialized, BaseAgent, settings, ConcatAgent
    
    if not agent_initialized or agent is None:
        logger.warning("ConcatAgent not initialized - attempting to initialize now...")
        ...
        if BaseAgent is None or settings is None:
            ...
            if not _retry_baseagent_import():
                ...
                raise ConfigurationError(
                    "ConcatAgent not initialized - BaseAgent import failed",
                    config_key="BASEAGENT_IMPORT"
                )
            
            # ConcatAgent should already be defined at module level if BaseAgent was imported
            # If it's still None, that means BaseAgent import succeeded but ConcatAgent wasn't defined
            # This shouldn't happen, but if it does, we can't define it here (would be a nested class)
            if BaseAgent is not None and ConcatAgent is None:
                logger.error("BaseAgent imported but ConcatAgent class not found - this should not happen")
                logger.error("ConcatAgent should be defined at module level when BaseAgent is imported")
                raise ConfigurationError(
                    "ConcatAgent class not found after BaseAgent import",
                    config_key="CONCATAGENT_CLASS"
                )
        
        # Try to initialize now
        if BaseAgent is not None and settings is not None:
            try:
                logger.info("Attempting to initialize ConcatAgent on-demand...")
                llm_config = settings.get_llm_config()
                minio_config = settings.get_minio_config()
                ...
                try:
                    from BaseAgent.registry import registry
                    registry.register(agent)
                    logger.info(f"✅ Registered ConcatAgent in BaseAgent registry (on-demand)")
                except ImportError:
                    try:
                        from TrinityAgent.BaseAgent.registry import registry
                        registry.register(agent)
                        logger.info(f"✅ Registered ConcatAgent in BaseAgent registry (on-demand, absolute import)")
                    except ImportError as e:
                        logger.warning(f"⚠️ Could not register agent in registry: {e}")
                
                agent_initialized = True
                logger.info("✅ ConcatAgent initialized successfully on-demand")
            except Exception as init_error:
                logger.error(f"❌ Failed to initialize ConcatAgent on-demand: {init_error}", exc_info=True)
                raise ConfigurationError(
                    f"ConcatAgent initialization failed: {str(init_error)}",
                    config_key="CONCATAGENT_INIT"
                )
        else:
    ...
    if not agent_initialized or agent is None:
        logger.error("ConcatAgent still not initialized after retry")
        raise ConfigurationError(
            "ConcatAgent not initialized",
            config_key="CONCATAGENT_INIT"
        )
    ...
    try:
        ...
        # Add concat_json if present
        if "concat_json" in result.data:
            concat_json = result.data["concat_json"]
            response["concat_json"] = concat_json
            ...
            # Add concat_config for frontend/backend
            response["concat_config"] = {
                "file1": file1,
                "file2": file2,
                "concat_direction": concat_json.get("concat_direction", "vertical"),
            }
        ...
        logger.error(f"CONCAT REQUEST FAILED (TrinityException): {e.message if hasattr(e, 'message') else str(e)}", exc_info=True)
        raise
    except Exception as e:
        # Wrap generic exceptions in AgentExecutionError
        logger.error(f"CONCAT REQUEST FAILED: {e}", exc_info=True)
...
@router.get("/concat/history/{session_id}")
...
logger.info("=" * 80)
logger.info("CONCAT AGENT MODULE LOADED")
logger.info(f"Router created: {router is not None}")
logger.info(f"Router type: {type(router)}")
