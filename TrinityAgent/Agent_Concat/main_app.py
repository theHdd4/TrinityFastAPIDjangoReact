...
def concatenate_files(request: ConcatRequest) -> Dict[str, Any]:
    ...
    if not agent_initialized or agent is None:
        ...
        if BaseAgent is None or settings is None:
            ...
            if not _retry_baseagent_import():
                ...
                return {
                    "success": False,
                    "error": "ConcatAgent not initialized - BaseAgent import failed",
                    "smart_response": "The concat agent is not available. BaseAgent could not be imported. Please check server logs for details.",
        ...
        if BaseAgent is not None and settings is not None:
            try:
                ...
                return {
                    "success": False,
                    "error": f"ConcatAgent initialization failed: {str(init_error)}",
                    "smart_response": "The concat agent could not be initialized. Please check server logs for details.",
                    "processing_time": round(time.time() - start_time, 2)
                }
        else:
    ...
    if not agent_initialized or agent is None:
        ...
        return {
            "success": False,
            "error": "ConcatAgent not initialized",
            "smart_response": "The concat agent is not available. Please check server logs.",
            "processing_time": round(time.time() - start_time, 2)
        }
