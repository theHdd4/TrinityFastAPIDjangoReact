...
1. Edit `TrinityBackendDjango/.env` and `TrinityFrontend/.env` to set the
   ...
   `OLLAMA_IP=10.2.4.48` either in `docker-compose.yml` or the environment to
   reach that server.
...
3. Copy `TrinityFrontend/.env.example` to `TrinityFrontend/.env`.
  ...
  both `https://trinity.quantmatrixai.com` and `http://10.2.4.48` work without
...
4. Finally ensure the browser isn't blocking third‑party cookies. Same‑site
  ...
  `http://10.2.4.48:8080`, `http://192.168.1.98:8081`,
  `http://172.17.48.1:8081`, `http://10.2.4.48:8081`,
