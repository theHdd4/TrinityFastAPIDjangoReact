...
class FileContextResolver:
    ...
    def _condense_metadata(
        ...
        stats_source_columns = highlight_columns or list(stats.keys())
        for col_name in stats_source_columns:
            ...
            if len(condensed_stats) >= 3:
                break

        if condensed_stats:
            summary["statistical_summary"] = condensed_stats

        sample_data = metadata.get("sample_data") or {}
        condensed_samples: Dict[str, List[Any]] = {}

        sample_columns = highlight_columns or summary.get("sample_columns", [])[:2]
        for col in sample_columns:
            sample = sample_data.get(col)
            if isinstance(sample, list) and sample:
                condensed_samples[col] = sample[:5]

        if condensed_samples:
            summary["sample_values"] = condensed_samples
