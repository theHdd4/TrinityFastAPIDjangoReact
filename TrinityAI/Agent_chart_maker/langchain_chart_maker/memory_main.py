...
from file_kv_store import FileBasedKeyValueStore
import uuid
...
MEMORY_STORE = FileBasedKeyValueStore("memory_store.json")
...
def generate_chart_json(request: ChartRequest):
    ...
    # Load or initialize memory and state from file
    session_data = MEMORY_STORE.get(session_id) or {"state": {}, "memory": []}
    prev_state = session_data.get("state", {})
    memory = session_data.get("memory", [])
    print(f"[Loaded Memory] {memory}")
    print(f"[Loaded State] {prev_state}")
    ...
    if missing:
        ...
        MEMORY_STORE.set(session_id, {"state": merged, "memory": memory})
        print("[Session] Saved incomplete state for next turn.")
        return {
            "success": False,
            "message": (
    ...
    print(f"[Final Chart JSON] {result}")
    print("[Session] Cleared session after completion.")
    return {
        "success": True,
