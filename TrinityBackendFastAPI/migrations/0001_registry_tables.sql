...
-- Apply same column updates to each tenant schema
DO $$
DECLARE
    ns RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER TABLE IF EXISTS %I.registry_project '
            || 'ADD COLUMN IF NOT EXISTS project_id BIGINT, '
            || 'ADD COLUMN IF NOT EXISTS user_id BIGINT, '
            || 'ADD COLUMN IF NOT EXISTS username TEXT, '
            || 'ADD COLUMN IF NOT EXISTS role TEXT, '
        ...
        EXECUTE format(
            'CREATE UNIQUE INDEX IF NOT EXISTS %I.registry_project_project_id_idx '
            || 'ON %I.registry_project(project_id);',
            ns.nspname,
            ns.nspname
        );
        EXECUTE format(
            'ALTER TABLE IF EXISTS %I.registry_session '
            || 'ADD COLUMN IF NOT EXISTS project_id BIGINT, '
            || 'ADD COLUMN IF NOT EXISTS user_id BIGINT, '
            || 'ADD COLUMN IF NOT EXISTS username TEXT, '
            || 'ADD COLUMN IF NOT EXISTS role TEXT, '
