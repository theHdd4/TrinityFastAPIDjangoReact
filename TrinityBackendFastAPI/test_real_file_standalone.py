"""
Standalone test of real file through U0, U1, U2 stages - no app imports.
Directly tests the file reading and processing logic.
"""
import sys
import os
import io
...
# Add only processor paths (avoid app initialization)
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'app', 'features', 'data_upload_validate', 'file_ingestion', 'detectors'))

# Import directly without going through app
from description_separator import DescriptionSeparator
from header_detector import HeaderDetector

# Import CSVReader directly
import importlib.util
csv_reader_path = os.path.join(os.path.dirname(__file__), 'app', 'features', 'data_upload_validate', 'file_ingestion', 'readers', 'csv_reader.py')
spec = importlib.util.spec_from_file_location("csv_reader", csv_reader_path)
csv_reader_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(csv_reader_module)
CSVReader = csv_reader_module.CSVReader
...
def test_u0_upload_stage(file_path):
    ...
    # Read using CSVReader (same as backend) in raw mode
    print(f"\n2. Reading file using CSVReader (raw mode, like backend)...")
    try:
        df_raw, metadata = CSVReader.read(
            content=file_bytes,
            filename=file_name,
        ...
        print(f"   [ERROR] CSVReader failed: {e}")
        import traceback
        traceback.print_exc()
        return None
    ...
    print(f"   First row (raw): {df_raw.iloc[0].tolist()[:5] if len(df_raw) > 0 else 'N/A'}...")
    if len(df_raw) > 1:
        print(f"   Second row (raw): {df_raw.iloc[1].tolist()[:5]}...")
    if len(df_raw) > 2:
        print(f"   Third row (raw): {df_raw.iloc[2].tolist()[:5]}...")
...
def test_u1_structural_scan_stage(u0_result):
    ...
    print(f"   Encoding: {u0_result['metadata'].get('encoding', 'unknown')}")
...
def test_u2_understanding_files_stage(u1_result):
    ...
    if description_rows:
        ...
        print(f"   [NOTE] No description rows detected")
...
def main():
    ...
    # Save response to JSON for inspection
    output_file = "test_u2_response.json"
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(u2_result, f, indent=2, ensure_ascii=False)
