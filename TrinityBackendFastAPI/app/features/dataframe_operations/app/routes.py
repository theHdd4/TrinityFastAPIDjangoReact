...
async def cached_dataframe(object_name: str):
    ...
    if not object_name.startswith(OBJECT_PREFIX):
        print(f"[DFOPS] object_name does not start with OBJECT_PREFIX: {OBJECT_PREFIX}")
        return Response(content='{"detail": "Invalid object_name prefix"}', status_code=400, media_type="application/json")
    # Try Redis first
    try:
...
async def list_saved_dataframes():
    ...
    try:
        files = []
        objects = minio_client.list_objects(MINIO_BUCKET, prefix=OBJECT_PREFIX, recursive=True)
        for obj in objects:
