"""
WebSocket handler for real-time collaborative Exhibition Mode synchronization.
...
from app.features.exhibition.deps import get_mongo_client

logger = logging.getLogger(__name__)

# Get the default database and collection names from environment
import os
...
class ExhibitionConnectionManager:
    """Manages WebSocket connections for collaborative exhibition editing."""
    ...
    def __init__(self):
        ...
        # Map of project_key -> object_id -> editor info (for slide object focus tracking)
        self.object_editors: Dict[str, Dict[str, dict]] = defaultdict(dict)
    ...
    async def connect(self, websocket: WebSocket, client_name: str, app_name: str, project_name: str, user_email: str = None, user_name: str = None, client_id: str = None):
        ...
        logger.info(
            f"Client {user_email or 'Anonymous'} connected to exhibition project {project_key}. "
    ...
    async def disconnect(self, websocket: WebSocket, client_name: str, app_name: str, project_name: str):
        ...
        logger.info(
            f"Client {user_data.get('email', 'Unknown') if user_data else 'Unknown'} disconnected from exhibition project {project_key}. "
    ...
    async def _debounced_save(
        ...
        try:
            ...
            if project_key in self.pending_states:
                ...
                logger.info(
                    f"💾 Persisting exhibition state for project {project_key} "
                    f"(cards: {len(latest_state.get('cards', []))})"
                )
                
                # Get collection manually (can't use Depends in websocket)
                mongo_client = get_mongo_client()
                database = mongo_client[DEFAULT_DATABASE]
                collection = database[LAYOUT_COLLECTION]
                ...
                result = await save_exhibition_list_configuration(
                    ...
                    exhibition_config_data={
                        ...
                        "slide_objects": latest_state.get("slide_objects", {}),
                        "mode": "exhibition",
                    },
                ...
                if result.get("status") == "success":
                    logger.info(
                        f"✅ Successfully persisted exhibition state for project {project_key} "
                        f"({result.get('documents_written', 0)} documents)"
                    )
                else:
                    logger.error(
                        f"❌ Failed to persist exhibition state for project {project_key}: "
            ...
            logger.info(f"Save task cancelled for exhibition project {project_key}")
        except Exception as e:
            logger.error(f"Error saving exhibition state for project {project_key}: {e}")
        finally:
    ...
    async def handle_card_update(
        ...
        if not card_id or not card_payload:
            logger.warning(f"Invalid card_update message for exhibition project {project_key}")
            return
        
        # Hydrate pending_states from MongoDB if empty
        if project_key not in self.pending_states or not self.pending_states[project_key].get("cards"):
            logger.info(f"🔄 Hydrating exhibition backend cache for project {project_key} from MongoDB")
            try:
                # Get collection manually (can't use Depends in websocket)
                mongo_client = get_mongo_client()
                database = mongo_client[DEFAULT_DATABASE]
                ...
                existing_record = await collection.find_one({**filter_query, "document_type": "layout_snapshot"})
                if not existing_record:
                    existing_record = await collection.find_one(filter_query)
                
                if existing_record:
                    existing_cards = existing_record.get("cards", [])
                    existing_slide_objects = existing_record.get("slide_objects", {})
                    self.pending_states[project_key] = {
                        "cards": existing_cards,
                        "slide_objects": existing_slide_objects
                    }
                    logger.info(f"✅ Hydrated {len(existing_cards)} cards from MongoDB for exhibition project {project_key}")
                else:
                    self.pending_states[project_key] = {"cards": [], "slide_objects": {}}
                    logger.info(f"⚠️ No existing config found, starting with empty state for exhibition project {project_key}")
            except Exception as e:
                logger.error(f"❌ Error hydrating cache for exhibition project {project_key}: {e}")
                self.pending_states[project_key] = {"cards": [], "slide_objects": {}}
    ...
    async def handle_full_sync(
        ...
        logger.info(f"📡 Full sync received for exhibition project {project_key}")
...
exhibition_manager = ExhibitionConnectionManager()


async def handle_exhibition_sync(
    websocket: WebSocket,
    client_name: str,
    app_name: str,
    project_name: str
    ...
    """
    WebSocket endpoint handler for exhibition synchronization.
    
    Manages real-time collaborative editing for a specific exhibition project.
    """
    # Initial connection without user info
    await exhibition_manager.connect(websocket, client_name, app_name, project_name)
    ...
    try:
        while True:
            ...
            if message_type == "connect":
                ...
                # Update user info
                project_key = exhibition_manager._get_project_key(client_name, app_name, project_name)
                previous_user_data = exhibition_manager.user_info.get(websocket)
                ...
                exhibition_manager.user_info[websocket] = user_data
                
                # Update active users list
                exhibition_manager.active_users[project_key] = [
                    u for u in exhibition_manager.active_users[project_key]
                    if u.get("client_id") not in {
                        user_data.get("client_id"),
                        previous_client_id,
                    }
                ]
                exhibition_manager.active_users[project_key].append(user_data)
                
                logger.info(
                    f"Client {user_email or 'Anonymous'} ({client_id}) connected to "
                    f"exhibition {client_name}/{app_name}/{project_name}"
                )
                ...
                await exhibition_manager._broadcast_user_list(client_name, app_name, project_name)
            
            elif message_type == "card_update":
                # Handle granular card-level update
                await exhibition_manager.handle_card_update(
                ...
                card_id = message.get("card_id")
                if card_id:
                    exhibition_manager.card_editors[project_key][card_id] = {
                    ...
                    await exhibition_manager.broadcast(message, client_name, app_name, project_name, exclude=websocket)
            
            elif message_type == "card_blur":
                # Handle user unfocusing from a card
                project_key = exhibition_manager._get_project_key(client_name, app_name, project_name)
                card_id = message.get("card_id")
                if card_id and card_id in exhibition_manager.card_editors[project_key]:
                    del exhibition_manager.card_editors[project_key][card_id]
                    # Broadcast blur event to other clients
                    await exhibition_manager.broadcast(message, client_name, app_name, project_name, exclude=websocket)
            
            elif message_type == "state_update":
                # Handle incremental state update
                await exhibition_manager.handle_state_update(
                    websocket, client_name, app_name, project_name, message
                )
            
            elif message_type == "full_sync":
                # Handle full state synchronization
                await exhibition_manager.handle_full_sync(
                    websocket, client_name, app_name, project_name, message
                )
        ...
        logger.info(f"Client disconnected from exhibition {client_name}/{app_name}/{project_name}")
    except Exception as e:
        logger.error(f"Error in exhibition WebSocket handler: {e}")
        try:
            await websocket.send_json({
                "type": "error",
        ...
        await exhibition_manager.disconnect(websocket, client_name, app_name, project_name)
