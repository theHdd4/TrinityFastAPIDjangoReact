...
def _store_result(atom_id: str, result: Dict[str, Any]) -> None:
    """Store computation result in Redis."""
    redis_client.setex((atom_id, "result"), UNPIVOT_CACHE_TTL, _ensure_redis_json(result))


def _load_result(atom_id: str) -> Optional[Dict[str, Any]]:
    """Load computation result from Redis."""
...
async def save_unpivot_result(atom_id: str, payload: UnpivotSaveRequest) -> UnpivotSaveResponse:
    """Save unpivot result to MinIO."""
    logger.info("Saving unpivot result for atom: %s", atom_id)
    
    result = _load_result(atom_id)
    if not result:
        raise HTTPException(status_code=404, detail="No unpivot data available to save")
    ...
    logger.info("Saved unpivot result for atom %s to %s", atom_id, minio_path)
