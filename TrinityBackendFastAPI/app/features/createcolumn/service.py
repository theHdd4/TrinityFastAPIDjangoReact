...
def _column_exists_case_insensitive(column_name: str, df: pd.DataFrame) -> bool:
    """Check if a column exists in the dataframe (case-insensitive).
    ...
    Returns:
        True if column exists (case-insensitive), False otherwise
    """
    column_name_lower = str(column_name).lower().strip()
    df_columns_lower = [str(col).lower().strip() for col in df.columns]
    return column_name_lower in df_columns_lower
    ...
    for op, columns, rename_val, op_idx in operations:
        ...
        if op == "add":
            ...
            if param == "fiscal_year":
                ...
                if _column_exists_case_insensitive(new_col, df):
            ...
            for item in metric_cols_list:
                ...
                # Check if column name already exists in dataframe (case-insensitive)
                if _column_exists_case_insensitive(new_col, df):
