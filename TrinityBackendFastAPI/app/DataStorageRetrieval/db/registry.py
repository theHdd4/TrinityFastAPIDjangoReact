...
async def register_project_session(data: dict) -> None:
    ...
    that both registry tables capture the latest project environment.  A
    ``ValueError`` is raised if any of these keys are missing.
    """
    if asyncpg is None:
        return
    conn = await asyncpg.connect(
    ...
    try:
        ...
        required = {"identifiers", "measures", "dimension_mapping"}
        missing = required - env_vars.keys()
        if missing:
            raise ValueError(
                "env_variables missing required keys: " + ", ".join(sorted(missing))
            )
