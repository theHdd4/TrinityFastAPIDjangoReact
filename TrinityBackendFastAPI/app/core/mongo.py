...
def build_host_mongo_uri(
    *,
    ...
    passing different ``username``/``password`` arguments.  When ``database``
    is supplied the URI points to that database directly so callers can rely on
    :meth:`motor.motor_asyncio.AsyncIOMotorClient.get_default_database`.
    """
