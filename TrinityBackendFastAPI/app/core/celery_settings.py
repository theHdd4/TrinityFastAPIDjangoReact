"""Celery configuration shim for the FastAPI service.

This mirrors the shared configuration provided by ``TrinityBackendDjango``
so the FastAPI app can bootstrap Celery even when the Django package is not
installed in the runtime environment (for example, lightweight API-only
containers).
"""
...
__all__ = ["configure_celery_app", "CELERY_CONFIG"]
